...
for map in range(0,maps):
	...
	for trial in range (0,trials):
		...
		for i in range(1,nodenumber+1):
			...
			verbatimrelative[i]=pending[i][i]
					...
					if pending[i][j]>0:
						...
						for k in connectmap[i]:
							if j not in arrival[k]:
								arrival[k][j]=time-verbatimrelative[j]
...
for i in range(0,maxtime+1):
	for j in range(1,maxhop+1):
		if i not in stat[j]:
			stat[j][i]=0
st=str(nodenumber)+"-"+str(propagation)+"-"+str(maps)+"-"+str(trials)+"testverbatim.csv"
with open(st,"w") as f:
	for i in range(0,maxtime+1):
		f.write(str(i))
		for j in range(2,maxhop+1):
